...
class ResBlock(torch.nn.Module):
    def __init__(self, in_channels, out_channels, *args, **kwargs ) -> None:
        ...
        self.block2 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=in_channels * 2, out_channels=in_channels * 2, kernel_size=(3,3), padding='same'),
            torch.nn.BatchNorm2d(in_channels * 2),
            torch.nn.GELU()
        )
        
        self.block3 = torch.nn.Sequential(
            torch.nn.Conv2d(in_channels=in_channels * 2, out_channels=out_channels, kernel_size=(3,3), padding='same'),
...
class ChannelAttention(torch.nn.Module):
    def __init__(self, dim_feedback, n_tx, n_rx, n_carrier, no_blocks, *args, **kwargs) -> None:
        ...
        self.input_conv = torch.nn.Conv2d(in_channels=2, out_channels=2, kernel_size=(3,3), padding = 'same')
        self.input_dense = torch.nn.Linear(in_features=n_tx*n_rx*n_carrier*2, out_features=dim_feedback)
        self.output_dense = torch.nn.Linear(in_features=dim_feedback, out_features=int(n_tx*n_rx*n_carrier*2))
        self.input_block_1 = ResBlock(in_channels=2, out_channels=2)
        self.unpool_1 = torch.nn.MaxUnpool2d(4)
        self.input_block_2 = ResBlock(in_channels=2, out_channels=2)
        self.unpool_2 = torch.nn.MaxUnpool2d(4)
    ...
    def forward(self,x):
        ...
        # x = self.unpool_1(x, output_size=(int(self.c/4), int(self.n_tx * self.n_rx/4)))
        x = self.input_block_2(x)
        x = self.attention_2(x)
        # x = self.unpool_2(x, output_size=(self.c, self.n_tx * self.n_rx))
