...
def ChannelCapacity(u, v, h, sigma):
    n_users = u.shape[1]
    nr = u.shape[2]
    d = u.shape[3]
    h = h[:,:,:,:,0] + 1j * h[:,:,:,:,1]
    ...
    channel_hermitian = h @ h_herm
    # rate_matrix = torch.eye(d).to(interference_term.get_device()) + torch.linalg.inv(interference_term) @ signal_term
    if torch.is_tensor(sigma):
        rate_matrix = torch.eye(channel_hermitian.shape[-1]).to(h_herm.get_device()) + (1/(n_users * sigma.unsqueeze(-1)**2)) * channel_hermitian
    else:
        rate_matrix = torch.eye(channel_hermitian.shape[-1]).to(h_herm.get_device()) + (1/(n_users * sigma**2)) * channel_hermitian
...
def SumRate_Noise(u, v, h, sigma):
    ...
    interference_term = interference_term.sum(dim=2)
    noise = sigma**2 * torch.eye(d).to(interference_term.get_device()).cfloat().unsqueeze(0).unsqueeze(0)
...
def SumRate_TX(u, v, h, sigma):
    ...
    if torch.is_tensor(sigma):
        sigma = sigma.unsqueeze(-1)
        interference_term = interference_term.sum(dim=2) + sigma**2 * torch.eye(nr).to(interference_term.get_device()).unsqueeze(0).unsqueeze(0)
    else:
...
def Interference_SU(u, v, h, sigma):
    ...
    h = rearrange(h, 'batch users nrx ntx -> batch 1 (users nrx) ntx')
